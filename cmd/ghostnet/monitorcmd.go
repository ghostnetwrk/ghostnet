...
import (
	...
	"github.com/ghostnetwrk/ghostnet/rpc"
...
var (
	...
	monitorCommand = cli.Command{
		Action: monitor,
		Name:   "monitor",
		Usage:  `GhostNet Monitor: node metrics monitoring and visualization`,
		Description: `
The GhostNet monitor is a tool to collect and visualize various internal metrics
...
func monitor(ctx *cli.Context) {
	...
	// Attach to an Ghostnet node over IPC or RPC
	...
	if len(monitored) == 0 {
		...
		if len(list) > 0 {
			...
			utils.Fatalf("No metrics collected by ghostnet (--%s).\n", utils.MetricsEnabledFlag.Name)
		}
	}
...
// retrieveMetrics contacts the attached ghostnet node and retrieves the entire set
