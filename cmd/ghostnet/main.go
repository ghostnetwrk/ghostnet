...
import (
	...
	"github.com/ghostnetwrk/ghostnet/accounts"
	"github.com/ghostnetwrk/ghostnet/cmd/utils"
	"github.com/ghostnetwrk/ghostnet/common"
	"github.com/ghostnetwrk/ghostnet/core"
	"github.com/ghostnetwrk/ghostnet/core/types"
	"github.com/ghostnetwrk/ghostnet/eth"
	"github.com/ghostnetwrk/ghostnet/ethdb"
	"github.com/ghostnetwrk/ghostnet/logger"
	"github.com/ghostnetwrk/ghostnet/logger/glog"
	"github.com/ghostnetwrk/ghostnet/metrics"
	"github.com/ghostnetwrk/ghostnet/params"
	"github.com/ghostnetwrk/ghostnet/rlp"
	"github.com/ghostnetwrk/ghostnet/rpc/codec"
	"github.com/ghostnetwrk/ghostnet/rpc/comms"
)

const (
	ClientIdentifier = "GHOSTNET"
	Version          = "0.1.0"
	VersionMajor     = 0
	VersionMinor     = 1
	VersionPatch     = 0
...
`,
		},
		{
			Action: version,
			Name:   "version",
			Usage:  "print ghostnet version numbers",
			Description: `
...
For non-interactive use the passphrase can be specified with the --password flag:
    ...
    ghostnet --password <passwordfile> account new
...
password to file or expose in any other way.
    ...
    ghostnet account update <address>
...
For non-interactive use the passphrase can be specified with the --password flag:
    ...
    ghostnet --password <passwordfile> account update <address>
...
changes.
    ...
    ghostnet account import <keyfile>
...
For non-interactive use the passphrase can be specified with the -password flag:
    ...
    ghostnet --password <passwordfile> account import <keyfile>
...
nodes.
		...
		},
		{
			Action: console,
			Name:   "console",
			Usage:  `GhostNet console: interactive JavaScript environment`,
			Description: `
The Geth console is an interactive shell for the JavaScript runtime environment
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://github.com/ghostnetwrk/ghostnet/wiki/Javascipt-Console
`},
		{
			Action: attach,
			Name:   "attach",
			Usage:  `GhostNet Console: interactive JavaScript environment (connect to node)`,
			Description: `
The Geth console is an interactive shell for the JavaScript runtime environment
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://github.com/ghostnetwrk/ghostnet/wiki/Javascipt-Console.
...
JavaScript API. See https://github.com/ghostnetwrk/ghostnet/wiki/Javascipt-Console
`,
		},
	}
	app.Flags = []cli.Flag{
...
func attach(ctx *cli.Context) {
	...
	if err != nil {
		utils.Fatalf("Unable to attach to ghostnet node - %v", err)
	}
...
func makedag(ctx *cli.Context) {
	args := ctx.Args()
	wrongArgs := func() {
		utils.Fatalf(`Usage: ghostnet makedag <block number> <outputdir>`)
	}
	switch {
	case len(args) == 2:
...
func gpubench(ctx *cli.Context) {
	args := ctx.Args()
	wrongArgs := func() {
		utils.Fatalf(`Usage: ghostnet gpubench <gpu number>`)
	}
	switch {
	case len(args) == 1:
