...
import (
	...
	"github.com/ghostnetwrk/ghostnet/rpc/api"
	...
	"github.com/ghostnetwrk/ghostnet/rpc/useragent"
...
var (
	...
	GasPriceFlag = cli.StringFlag{
		...
		Value: new(big.Int).Mul(big.NewInt(50), common.Ggnt).String(),
	}
	...
	GpoMaxGasPriceFlag = cli.StringFlag{
		...
		Value: new(big.Int).Mul(big.NewInt(500), common.Ggnt).String(),
	}
...
func MakeEthConfig(clientID, version string, ctx *cli.Context) *eth.Config {
	...
	if err != nil {
		glog.V(logger.Error).Infoln("WARNING: No ghostbase(coinbase) set and no accounts found as default")
	}
...
func IpcSocketPath(ctx *cli.Context) (ipcpath string) {
	if runtime.GOOS == "windows" {
		...
		ipcpath = common.DefaultIpcPath()
		if ctx.GlobalIsSet(DataDirFlag.Name) {
			ipcpath = filepath.Join(ctx.GlobalString(DataDirFlag.Name), "ghostnet.ipc")
		}
		if ctx.GlobalIsSet(IPCPathFlag.Name) {
			ipcpath = ctx.GlobalString(IPCPathFlag.Name)
		}
